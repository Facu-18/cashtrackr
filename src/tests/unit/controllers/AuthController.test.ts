...
describe('AuthController.createAccount', () => {
    ...
    it('should return a 409 status and an error message if the email is already registered', async () => {
        ...
        expect(data).toHaveProperty('error', 'Ya existe un usuario con ese email')
        expect(User.findOne).toHaveBeenCalled()
        expect(User.findOne).toHaveBeenCalledTimes(1)
    })

    it('should register a new user and return succes message', async () => {
        const req = createRequest({
            method: 'POST',
            url: '/api/auth/create-account',
            body: {
...
describe('AuthController.login', () => {
    ...
    it('should return 403 if the account has not been confirmed', async () => {
        ...
        expect(data).toEqual({error: 'Debes confirmar la cuenta'})
    })
